...
func dumpAnimationEventHashes(a *app.App, fileID stingray.FileID) error {
	...
	for _, eventHash := range stateMachine.AnimationEventHashes {
...
func main() {
	...
	files, err := a.MatchingFiles("", "", []string{"state_machine"}, nil, "", prt.Infof)
	if err != nil {
		prt.Fatalf("%v", err)
	}
	...
	for fileID := range files {
		...
		if err := dumpAnimationEventHashes(a, fileID); err != nil {
			if errors.Is(err, context.Canceled) {
				prt.NoStatus()
