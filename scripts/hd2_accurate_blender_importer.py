...
def load_shaders(resource_path: str) -> Tuple[ModuleType, Material, Material, Material, Material, Material, Material, Material]:
    ...
    return shader_module, shader_mat, skin_mat, lut_skin_mat, building_mat, concrete_mat, fence_mat, triplanar_illuminate_building_mat, illuminate_building_mat
    ...
    for primIdx, primitive in enumerate(mesh["primitives"]):
        for varIdx in range(len(variants)):
            ...
            if materialIndex in materialTextures:
                ...
                if len(material["extras"]) == 0 or not any((is_pbr, is_tex_array_skin, is_lut_skin, is_lut, is_building, is_concrete, is_fence, is_illuminate_building_triplanar, is_illuminate_building_monoplanar)):
                    continue
...
def main():
    ...
    shader_module, shader_mat, skin_mat, lut_skin_mat, building_mat, concrete_mat, fence_mat, il_building_mat_triplanar, il_building_mat = load_shaders(resource_path)
    ...
    for node in gltf["nodes"]:
        ...
        if "mesh" in node:
            convert_materials(gltf, node, variants, hasVariants, materialTextures, args.packall, shader_module, shader_mat, skin_mat, lut_skin_mat, building_mat, concrete_mat, fence_mat, il_building_mat_triplanar, il_building_mat, unused_texture, unused_secondary_lut)
